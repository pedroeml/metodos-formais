...
next
  fix y::nat
  assume HI:"\<forall>x. soma x y = x + y"
  show "\<forall>x. soma x (Suc y) = x + (Suc y)"
  proof (rule allI)
    fix x::nat
...
next
  fix y::nat
  assume HI:"\<forall>x. soma x y = soma y x"
  show "\<forall>x. soma x (Suc y) = soma (Suc y) x"
  proof (rule allI)
    fix x::nat
    have "soma x (Suc y) = Suc (soma x y)" by (simp only:somaeq2)
...
next
  fix y::nat
  assume HI:"\<forall>x. soma x (Suc y) = soma (Suc x) y"
  show "\<forall>x. soma x (Suc (Suc y)) = soma (Suc x) (Suc y)"
  proof (rule allI)
    fix x::nat
